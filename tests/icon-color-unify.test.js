...
describe('icon color unification', () => {
  test('핵심 아이콘 버튼 배경은 검은색이고 크기는 + 버튼(26px)과 같아야 한다', () => {
    ...
    expect(legacy).toMatch(/\.sync-main-btn\.sync-gear-btn\s*\{[\s\S]*?width:\s*30px;[\s\S]*?height:\s*30px;[\s\S]*?background:\s*#000;/);
  ...
  test('내부 아이콘 글리프 크기도 동일 비율(14px)이어야 한다', () => {
    ...
    expect(legacy).toMatch(/\.sync-gear-icon\s*\{[\s\S]*?width:\s*15px;[\s\S]*?height:\s*15px;/);
